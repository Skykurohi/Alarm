...
svw_path      = 'M:\\Projects\\Development\\Alarm\\tools\\SoundVolumeView.exe'
svw_target    = 'Headphones'
...
def main():
    execute(f'{svw_path} /SetVolume "{svw_target}" {initial_volume:.1f}')
    ...
    if initial_volume < final_volume:
        ...
        while x <= steps:
            ...
            execute(f'{svw_path} /SetVolume "{svw_target}" {volume:.1f}')
            time.sleep(4)
            x += 1
